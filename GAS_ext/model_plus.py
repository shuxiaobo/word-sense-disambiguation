...
class Model:
    def __init__(self, config, n_senses_from_target_id, init_word_vecs=None):
        ...
        n_hypo = config.n_hypo
        n_hyp = n_hyper + n_hypo + 1

        embedding_size = config.embedding_size
        n_units = config.n_lstm_units
        gloss_emb_size = config.gloss_emb_size
        ...
        self.glosses = tf.placeholder(tf.int32, shape=[batch_size, max_n_sense, n_hyp, max_gloss_words], name='glosses')
        self.glosses_lenth = tf.placeholder(tf.int32, shape=[batch_size, max_n_sense, n_hyp], name='glosses_lenth')
        ...
        with tf.variable_scope('sentence', initializer=lstm_initializer):
            ...
            inputs_context = tf.nn.embedding_lookup(word_embeddings, self.inputs)  # [batch_size, n_step, dim]
            inputs_s = inputs_context
            ...
            s_outputs, s_final_state = tf.nn.bidirectional_dynamic_rnn(s_cell_fw,
                                                                       ...
                                                                       dtype=tf.float32,
                                                                       time_major=False)
            # s_output = tf.concat([s_outputs[0][:, n_step_f - 1, :], s_outputs[1][:, n_step_f + 1, :]], 1)
        ...
        with tf.variable_scope('gloss_bilstm_embed', initializer=lstm_initializer):
            g_cell_fw = lstm_cell(gloss_emb_size)  # forward
            g_cell_bw = lstm_cell(gloss_emb_size)  # backward
            inputs_g = tf.reshape(self.glosses, [batch_size * max_n_sense * n_hyp, max_gloss_words])
            inputs_g = tf.nn.embedding_lookup(word_embeddings,
                                              inputs_g)  # [batch_size*max_n_sense*n_hyp, max_gloss_words, dim]
            inputs_g = tf.nn.dropout(inputs_g, keep_prob)
            sequence_length = tf.reshape(self.glosses_lenth, [batch_size * max_n_sense * n_hyp])
            ...
            g_outputs_bw = g_outputs[1]  # [batch_size*max_n_sense*n_hyp, n_step, output_size]
            g_output = [g_outputs_fw[:, -1, :], g_outputs_bw[:, 0, :]]
            g_output = tf.concat(g_output, 1)  # [batch_size*max_n_sense*n_hyp, 2*gloss_emb_size]

        with tf.variable_scope('gloss_hyp', initializer=lstm_initializer):
            inputs_hg = tf.reshape(g_output, [batch_size * max_n_sense, n_hyp, 2 * gloss_emb_size])
            with tf.variable_scope('forward'):
                hg_cell_fw = lstm_cell(n_units)  # forward
                inputs_hyper = inputs_hg[:, :n_hyper + 1, :]  # hyper3 hyper2 hyper1 target
                hyper_length = tf.reshape(self.hyper_lenth, [batch_size * max_n_sense])
                inputs_hyper = tf.nn.dropout(inputs_hyper, keep_prob)
                hyper_outputs, hyper_final_states = tf.nn.dynamic_rnn(hg_cell_fw, inputs_hyper,
                                                                      sequence_length=hyper_length, dtype=tf.float32)
                # hyper_outputs: [batch_size*max_n_sence, n_hyp, n_units]
                hyper_final_output = hyper_outputs[:, -1, :]
            with tf.variable_scope('backward'):
                hg_cell_bw = lstm_cell(n_units)  # backward
                inputs_hypo = tf.reverse(inputs_hg[:, n_hyper:, :], [1])  # target hypo1 hypo2 hypo3 => hypo3 hypo2 ...
                hypo_length = tf.reshape(self.hypo_lenth, [batch_size * max_n_sense])
                inputs_hypo = tf.nn.dropout(inputs_hypo, keep_prob)
                hypo_outputs, hypo_final_states = tf.nn.dynamic_rnn(hg_cell_bw, inputs_hypo,
                                                                    sequence_length=hypo_length, dtype=tf.float32)
                # hypo_outputs: [batch_size*max_n_sence, n_hyp, n_units]
                hypo_final_output = hypo_outputs[:, -1, :]  # [batch_size*max_n_sence, n_units]

            g_output = tf.concat([hyper_final_output, hypo_final_output], 1)  # [batch_size*max_n_sence, 2*n_units]

            if config.concat_target_gloss:
                g_output = tf.concat([g_output, inputs_hg[:, n_hyper, :]], 1)  # [batch_size*max_n_sence, 4*n_units]
                g_output = tf.reshape(g_output, [batch_size, max_n_sense, 4 * n_units])
            else:
